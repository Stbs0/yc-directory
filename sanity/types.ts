...
// Query: *[_type == "startup" && defined(slug.current) && !defined($search) || name match $search || category match $search || author->name match $search] | order(_createdAt desc) {    _id,    name, // Renamed from title to match schema    slug { current },    _createdAt,    author->{      _id,      name,      image,      bio    },    views,    description,    category,    image,    pitch // Added pitch to match schema  }
...
// Query: *[_type == "startup" && _id == $id] [0]{    _id,    name,     slug { current },    _createdAt,    author->{      _id,      name,      image,      bio,      username    },    views,    description,    category,    image,    pitch   }
...
// Query: *[_type == "startup" && author._ref == $id] | order(_createdAt desc) {    _id,    name, // Renamed from title to match schema    slug { current },    _createdAt,    author->{      _id,      name,      image,      bio    },    views,    description,    category,    image,    pitch // Added pitch to match schema  }
